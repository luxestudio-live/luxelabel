        ...
        <div className="bg-white dark:bg-card p-8 rounded-xl shadow-lg border border-border/30 w-full md:w-1/2 mx-auto">
          ...
          <div className="flex gap-2 mb-8 justify-center">
            {TABS.map((t, i) => (
              <button
                ...
                onClick={() => { setTab(i); setOtpSent(false); setErrors({}); setFullName(""); setPhone(""); setOtp(""); setAgree(false); }}
              >
                {t}
              </button>
            ))}
          ...
          {tab === 0 && (
            <form className="space-y-4" onSubmit={handleEmailRegister}>
              <div>
                <input type="text" placeholder="Full Name" className="border p-3 rounded w-full" value={fullName} onChange={e => setFullName(e.target.value)} required />
              ...
              <div>
                <input type="password" placeholder="Confirm Password" className="border p-3 rounded w-full" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} required />
